...
**Contract name:** ZexdexAppLottery  
...
View [ZexdexAppLottery.sol on BscScan](https://bscscan.com/address/0x5aF6D33DE2ccEC94efb1bDF8f92Bd58085432d2c#code).

View the [ZexdexApp: Lottery contract on BscScan](https://bscscan.com/address/0x5aF6D33DE2ccEC94efb1bDF8f92Bd58085432d2c).

## Audits

The ZexdexApp Lottery has been audited twice so far. View the results below:

[Peckshield's Lottery Audit](https://github.com/peckshield/publications/blob/master/audit_reports/PeckShield-Audit-Report-zexdexLottery-v1.0.pdf)

[Slowmist's Lottery Audit](https://github.com/slowmist/Knowledge-Base/blob/master/open-report/Smart%20Contract%20Security%20Audit%20Report%20-%20ZexdexApp%20Lottery.pdf)
...
```text
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInZex;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] zexPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInZex;
        uint32 finalNumber;
```
...
| `priceTicketInZex`        | uint256      | The price of a ticket in ZEX \(approximately $5 USD\).                                                                                                                                                                |
...
| `zexPerBracket`           | uint256\[6\] | The amount of ZEX to distribute to winners of each bracket.                                                                                                                                                           |
...
| `amountCollectedInZex`    | uint256      | The amount of ZEX collected through ticket sales for the Lottery round.                                                                                                                                               |
...
| `priceTickets`    | uint256 | The price of a ticket in ZEX. |
...
| :---------- | :------ | :-------------------------------- |
| `lotteryId` | uint256 | The id of the Lottery.            |
| `amount`    | uint256 | Amount, in ZEX token, to inject. |
...
| `priceTicketInZex` | uint256      | Price of a ticket in ZEX.                                 |
...
Allows admin to recover incorrect tokens sent to address mistakenly. Cannot be ZEX tokens.
...
### setMinAndMaxTicketPriceInZex

```text
function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner;
```

Allows admin to set upper and lower limit of ticket price in ZEX value. Minimum price must be lower than maximum price.

| Name                   | Type    | Description                |
| :--------------------- | :------ | :------------------------- |
| `minPriceTicketInZex` | uint256 | The minimum price in ZEX. |
| `maxPriceTicketInZex` | uint256 | The maximum price in ZEX. |
...
Emitter: `startLottery` [go to startLottery](lottery.md#startlottery)
...
Emitter: `drawFinalNumberAndMakeLotteryClaimable` go to [drawFinalNumberAndMakeLotteryClaimable](lottery.md#drawfinalnumberandmakelotteryclaimable)
...
Emitter: `setOperatorAndTreasuryAndInjectorAddresses` [go to setOperatorAndTreasuryAndInjectorAddresses](lottery.md#setoperatorandtreasuryandinjectoraddress)
...
Emitter: `changeRandomGenerator` [go to changeRandomGenerator](lottery.md#changerandomnumbergenerator)
